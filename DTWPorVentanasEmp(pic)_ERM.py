...
SAVEPATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 
                        "DTWPorVentanasEmp(pic)_ERM") # Especificacion de carpeta para almacenar los graficos
CSV_NAME = "PruebaPorVentanasEmp(picos)_ERM.csv" # Nombre del archivo CSV
...
for filename in tqdm(FILES, desc=f'Porcentaje de avance'):
    ...
    for i in range(len(calibrations)):
        if (best_ERM > calibrations[i].ErM):
            best_ERM = calibrations[i].ErM
...
print(f"Datos de ejecución '{csv_path}' guardados exitosamente.")
